...
/**
 * Estimate the impact of \xi (pattern similarity threshold) parameter in the TPCA model
 * proposed in paper:
 * W. Gong, Trajectory Pattern Change Analysis in Campus WiFi Networks, MobiGIS'2013
 *
 */
public class estimate_xi {
	//************ SETTINGS START **************
	...
	public static void main(String [] args) throws IOException{	
		...
		miningPatternChange(um, "/Users/chenxm/Jamin/Datasets/SJTU/changedata-xi.txt");
	...
	public static void miningPatternChange(UserManager um, String outputfile) throws IOException{
		...
		for ( String uid : um.getAllUsers()) {
			...
			for ( double graph_minsim = 0.02; graph_minsim < 0.82; graph_minsim += 0.02){
				System.out.println("graph minsim: "+ graph_minsim);
				...
				for ( int i = 1; i < patternSets.size(); i++){
					if ( patternSets.get(i).size() > 0){
						...
						for ( int j = i-5; j < i; j++){
							...
							if ( j >= 0 && patternSets.get(j).size() > 0 ){
								double cinfo = PatternSetChange.changeInformationMeasure(patternSets.get(j), 
										patternSets.get(i), graph_minsim);
						...
						if (weightTotal != 0){
							...
							or.append(String.format("%.4f ", cinfoTotal / weightTotal));
						}
						else {
							r.append("*/0");
							or.append("/0 ");
