package omnilab.bd.pattern.spmf.BIDE_and_prefixspan;
...
public class PseudoSequence {
	 ...
	 * @return the item.
	 */
	protected Integer getItemAtInItemsetAt(int indexItem, int indexItemset) {
	 ...
	 * @return the itemset
	 */
	protected Itemset getItemset(int index) {
	...
	/**
	 * Check if an item inside an itemset is postfix related to the given oldPrefix
	 * This method is to decide the item flag when finding frequent items 
	 * without generating additional sequences.
	 * If itemset was cut by projection, this method return the same result with isPostfix().
	 * Otherwise, it will check if the itemset contains the last itemset of oldPrefix.
	 * @param indexItemset The index of itemset inside a sequence
	 * @param indexItem The index of an item inside an itemset
	 * @param oldPrefix The previous prefix.
	 * @return boolean flag
	 */
	protected boolean isPostfixOfOldPrefix(int indexItemset, int indexItem, SequentialPattern oldPrefix){
		// first perform check with isPostfix() method.
		if ( this.isPostfix(indexItemset) )
				return true;
		// get last itemset of oldPrefix
		Itemset prefixLastItemset = oldPrefix.getItemsets().get(oldPrefix.size()-1);
		// 
		Itemset cloneItemset = new Itemset();
		for ( int i = 0; i < indexItem; i++ ){
			cloneItemset.addItem(this.getItemset(indexItemset).get(i));
		}
		if (cloneItemset.containsAll(prefixLastItemset))
			return true;
		return false;
	}
